...
import static com.example.userservice.persistence.entity.Role.ROLE_ADMIN;
import static com.example.userservice.persistence.entity.Role.ROLE_STUDENT;
...
public class UserServiceImplTest {
    ...
    // ✅ ADD THIS - Mock KeycloakUserService to prevent NullPointerException
    @Mock
    ...
    void setUp() {
        signUpRequest = SignUpRequest.builder()
                ...
                .role(String.valueOf(ROLE_STUDENT))
                .build();
    ...
    void createUser_Success() {
        ...
        // ✅ Mock getUserByUsername - called first to check if user exists in Keycloak
        when(keycloakUserService.getUserByUsername(anyString())).thenReturn(null);

        // ✅ Mock createKeycloakUser - with CORRECT parameters (firstName, lastName, no telephone)
        when(keycloakUserService.createKeycloakUser(
                anyString(),   // username
                anyString(),   // email
                anyString(),   // password
                anyString(),   // firstName (NOT fullName!)
                anyString(),   // lastName (NOT telephone!)
                any(Role.class) // role
        ...
        // ✅ Verify getUserByUsername was called first
        verify(keycloakUserService).getUserByUsername(eq("testuser"));

        // ✅ Verify createKeycloakUser with CORRECT parameters
        verify(keycloakUserService).createKeycloakUser(
                eq("testuser"),
                eq("test@example.com"),
                eq("password123"),
                eq("Test"),        // ✅ firstName (first word of "Test User")
                eq("User"),        // ✅ lastName (rest of "Test User")
                eq(ROLE_STUDENT)
        );
    }
    ...
    void createUser_UsernameAlreadyExists_ThrowsException() {
        ...
        verify(keycloakUserService, never()).createKeycloakUser(
    ...
    void getAllUsers_ReturnsListOfUsers() {
        ...
        assertEquals(Role.ROLE_STUDENT, dto1.getRole());  // ✅ Right - comparing enum to enum
